...
int delay_run_sound = 1000; // length of the run sound in ms - not important, because we set loop mode
int delay_boost_sound = 1000; // length of the boost sound in ms - not important, because we set loop mode
  ...
  if (buttonState[0] == 0) {                                    //if the first button is pressed
    if (debounce[0] == 0) {                                     //if the debounce flag is not set
      ...
      switch (fire.getState()) {
        ...
        case (fire.States::SHUTDOWN):                           //if the fire is in SHUTDOWN
          fire.setState(fire.States::STARTUP);                  //  set fire to STARTUP
          timer = millis()-1;                                   //  set startup delay to already expired
        default:
          break;
#ifdef DEBUG
    ...
    if (debounce[0] == 1) {
      ...
      if (fire.getState() != fire.States::OFF                   //if the fire is not in OFF
        ...
        fire.setState(fire.States::SHUTDOWN);                   //  turn it OFF
        timer = millis() + delay_shutdown_sound;                //  set timer
  ...
  switch (fire.getState()) {
    case fire.States::STARTUP:                                  //if the fire is STARTING UP
      ...
      if (millis() >= timer) {                                  //  if the timer is up (STARTUP sound has completed playing)
        fire.setState(fire.States::RUN);                        //  set the fire state to RUN
        if (!mute) {                                            //  if mute is not true
    ...
    case fire.States::BOOST:                                          //if fire is BOOST
      ...
      if (millis() > fire.delay_boost) {                              //  if boost timer is expired
        fire.setState(fire.States::RUN);                              //    set state to RUN
    ...
    case fire.States::SHUTDOWN:                                       //if fire is SHUTDOWN
      ...
      if (millis() > timer) {                                         //  if timer has expired
        fire.setState(fire.States::OFF);                              //    set state to OFF
      }
      break;
